...
use think\Model;
class DBlssdjt extends Model
{
    protected $connection = 'DB_Config_lssdjt';
}
class Lssdjt extends Base
{
    public function index($m='',$d=''){
		...
		$datas=DBlssdjt::table('Content')->where('月',$m)->where('日',$d)->order('年','desc')->select();
		$data=DBlssdjt::table('Content')->where('月',$m)->where('日',$d)->order('年','desc')->select();
		$lists=[];
		$img='';
		foreach ($data as $row){
		...
		return $this->fetch('index/lssdjt');
    }
    public function view($id='')
    {
		$data=DBlssdjt::table('Content')->where('id',$id)->find();
