...
$m_x = ($m_width - $m_fontBox[2]) / 2 + 15;  // 获取字符宽度并居中显示


// 输出第一行
// 输入：句柄、字体大小、角度、坐标点x、y、颜色65、字体文件、字符串 (int im, int size, int angle, int x, int y, int col, string fontfile, string text);
imagettftext($m_im, $maker['font_size1'], 0, $m_x, 53, $m_color, $maker['font1'], $maker['str1']);  // 在图片上绘制文字
...
imagettftext($m_im, $maker['font_size2'], 0, $m_x, 355, $m_color, $maker['font1'], $maker['str2']);  // 在图片上绘制文字
